...
<p>
Para visualizar la actualización de las medidas frente al COVID-19: [Base de Seguimiento Nacional](https://docs.google.com/spreadsheets/d/e/2PACX-1vRJsBU46m84Yl8hUIo5c1fILeYqcSa4vDXhFyIQ_QbUJndBfLLO-N4nkHOfl5hgZMWKzv1L73oiqwzm/pubhtml?gid=31875411&single=true)
</p>
...
```{r codigo redes, echo=FALSE,eval=TRUE,message=FALSE,warning=FALSE, fig.width=3.5}
library(tidygraph)
library(networkD3)
library(igraph)
library(visNetwork)
...
colnames(actores) <- c("id","Actor","Nivel")  


enlaces<-read.csv(enlacesLink)
colnames(enlaces) <- c("from","to","type.relation")

visNetwork(actores, enlaces, width = "100%")
...
visNetwork(actores, enlaces, width = "100%") %>% 
  visNodes(shape = "square", 
           color = list(background = "lightblue", 
                        border = "darkblue",
                        highlight = "yellow"),
           shadow = list(enabled = TRUE, size = 0)) %>%
  visLayout(randomSeed = 12) 

actores1 <- data.frame(actores, 
                    shape = ifelse(actores$Nivel=="Nacional","circle", "square"))
actores1<-actores1 %>% rename(label=Actor) 
actores1<-actores1 %>% mutate(font.size = 10*3)

visNetwork(actores1, enlaces, width = "100%" ,physics=T, idToLabel=T) %>% 
...
```{r Red total, fig.width=3.5}
#redtotal<-simpleNetwork(red_nacional,linkColour = "grey",opacity = 1,nodeColour = "black",) 
#redtotal
```
