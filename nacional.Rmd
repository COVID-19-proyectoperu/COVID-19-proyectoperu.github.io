...
sum_protesta %>% ggplot(aes(x=Fecha, y=acumulado, fill=Area, color=Area)) +  geom_line(size=0.5) +  geom_point(size = 2) + theme_classic() + ylim(0,30) + theme(plot.caption=element_text(size=8, hjust=0.5, face="italic", color="black")) + scale_x_date(date_breaks = "1 week", date_labels = "%m-%d")+labs(title="Total de protestas registradas durante el \n Estado de Emergencia",caption = "Fuente: Base de Seguimiento Nacional. Elaborado para 'Politólogos en cuarentena'" ,y = "Número de protestas", x="Fecha")+ theme(plot.caption=element_text(size=8, hjust=0.5, face="italic", color="black")) 
