...
tools="https://docs.google.com/spreadsheets/d/e/2PACX-1vRJsBU46m84Yl8hUIo5c1fILeYqcSa4vDXhFyIQ_QbUJndBfLLO-N4nkHOfl5hgZMWKzv1L73oiqwzm/pub?gid=31875411&single=true&output=csv"

instrumentos<-as.data.frame(read.csv(tools))
instrumentos$Fecha<-as.Date(instrumentos$Fecha)
```
...
<p>
Para visualizar la actualización de las medidas frente al COVID-19: [Monitoreo de la toma de decisiones](https://docs.google.com/spreadsheets/d/e/2PACX-1vRJsBU46m84Yl8hUIo5c1fILeYqcSa4vDXhFyIQ_QbUJndBfLLO-N4nkHOfl5hgZMWKzv1L73oiqwzm/pubhtml?gid=379947845&single=true)
</p>

```{r, echo=FALSE,eval=TRUE}
library(ggplot2)
q<-ggplot(instrumentos, aes(x=Fecha, y=Area,color=Area)) +  geom_jitter(size=2) +   scale_x_date(date_breaks = "1 week", date_labels = "%m-%d") + theme(axis.text.x  =element_text(angle = 70, hjust = 1)) +geom_vline(aes(xintercept=(as.numeric(as.Date("2020-03-15")))),  linetype = 2) + geom_text(aes(x=(as.Date("2020-03-15")), y= "Economía", label='Inicio del Estado de emergencia'), size=2.8, angle=90, vjust=-0.4, hjust=0,color="black") +geom_vline(aes(xintercept=(as.numeric(as.Date("2020-03-18")))),  linetype = 2) + geom_text(aes(x=(as.Date("2020-03-18")), y= "Economía", label='Inmoviilizacion Social Obligatoria'), size=2.8, angle=90, vjust=-0.4, hjust=0,color="black") +geom_vline(aes(xintercept=(as.numeric(as.Date("2020-03-31")))),  linetype = 2) + geom_text(aes(x=(as.Date("2020-03-31")), y= "Economía", label='Primera prórroga hasta el 12 de abril '), size=2.8, angle=90, vjust=-0.4, hjust=0, color="black")+geom_vline(aes(xintercept=(as.numeric(as.Date("2020-04-13")))),  linetype = 2) + geom_text(aes(x=(as.Date("2020-04-13")), y= "Economía", label='Segunda prórroga hasta el 26 de abril '), size=2.8, angle=90, vjust=-0.4, hjust=0, color="black") + labs(title="Trazabilidad de las decisiones por sector",caption = "Fuente: El Peruano. Elaborado para 'Politólogos Infectados'" ,x = "Fecha", y="Medidas adoptadas")+ theme(plot.caption=element_text(size=8, hjust=0.5, face="italic", color="black")) 

q+theme(axis.title.y=element_blank(),axis.text.y=element_blank(), axis.ticks.y=element_blank() , legend.position = "left")
```
...
### 2.5. Conflictos sociales en torno al COVID-19

### 2.6. Noticias, demandas y pronunciamientos
