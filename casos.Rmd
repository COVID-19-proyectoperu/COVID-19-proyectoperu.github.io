...
hospital<-casos[!is.na(casos$hospital_nuevos),]


### Evolucion de hospitalizados

g5<-hospital %>% ggplot(aes(x=dia, y=hospital_nuevos))+ geom_line(color="grey", size=0.8) +  geom_point(size=2, color="black") + theme_minimal() + labs(title= "Evolución de nuevos de nuevos hospitalizados por dia",x = "Fecha", y="Número de hospitalizados",caption = "Fuente: Reportes diarios del Minsa. Elaborado para 'Politólogos en cuarentena'")+ theme(plot.caption=element_text(size=8, hjust=0.5, face="italic", color="black")) 
...
### 1.6. Evolución de personas en UCI

```{r UCI, warning=FALSE, echo=FALSE,eval=TRUE}
ucidata<-casos[!is.na(casos$uci_uso),]

uci<-ucidata %>% ggplot(aes(x=dia, y=uci_uso))+ geom_line(color="blue", size=0.8) +  geom_point(size=2, color="blue") + theme_minimal() + labs(title= "Evolución de personas en UCI",x = "Fecha", y="Número de personas en UCI",caption = "Fuente: Sala Situacional - Minsa. Elaborado para 'Politólogos en cuarentena'")+ theme(plot.caption=element_text(size=8, hjust=0.5, face="italic", color="black")) + geom_text(aes(y=uci_uso,label=uci_uso),position=position_dodge(width = 0.9),size=2.5, vjust=-1,hjust=0.5,col="black")

uci

```

### 1.7. Evolución en la disponibilidad de camas UCI
...
library(dplyr)
uci_semana%>%
  dplyr::select("Fecha"="dia","Total de camas UCI"="uci_total" ,"Camas UCI ocupadas"="uci_uso", "Camas UCI disponibles"="uci_disponibles","% de aumento respecto al día anterior"="x_uci") %>%  DT::datatable(rownames = FALSE)
```




### 1.8. Evolución de los casos positivos activos
...
g4<-abril %>% ggplot(aes(x=dia, y=casos_activos)) +  geom_area(alpha=0.1) +  geom_line(color="#56B4E9", size=0.8) +  geom_point(size=2, color="#56B4E9") + theme_minimal() + labs(title="Numero de casos activos (Abril)",caption = "Fuente: Sala Situacional - Minsa. Elaborado para 'Politólogos en cuarentena'" ,x = "Fecha", y="Número de casos")+ theme(plot.caption=element_text(size=8, hjust=0.5, face="italic", color="black"))
