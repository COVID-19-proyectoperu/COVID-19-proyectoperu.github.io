...
casos %>%
  ggplot(aes(x = dia)) +
  ylim(0,max(casos$casos_activos)*2.5)+geom_bar(fill="#c93c20",aes(y=c_positivos_dia), stat = "identity")+geom_line(color="#f4fc00",aes(y =casos_activos), size=2) + geom_line(color="#42857b",aes(y=recuperados_dia))+geom_area(color="#3f888f",aes(y=recuperados_dia),alpha=0.6) +geom_point(aes(y = recuperados_dia), col = "#42857b")+ scale_y_continuous(trans = "log10")  + labs(title="Evolución de casos positivos activos",caption = "Fuente: Sala Situacional - Minsa. Elaborado para 'Politólogos en cuarentena'" ,x = "Fecha", y="Número de casos")+ theme(plot.caption=element_text(size=8, hjust=0.5, face="italic", color="black")) + scale_x_date(date_breaks = "1 week", date_labels = "%m-%d")+
  theme_classic() + geom_text(aes(y=casos_activos,label=casos_activos),position=position_dodge(width = 0.9),size=2.5, vjust=1,hjust=1.2,col="black", angle=90) 
...
g2 + geom_text(aes(y=c_positivos_dia,label=c_positivos_dia),position=position_dodge(width = 0.9),size=2.5, vjust=0,hjust=0.5,col="black", angle=-90)
...
popup_loreto <- paste0("<b>", "Departamento: ", "</b>", as.character(loretocondata$DEPARTAMENTO), "<br>",
                "<b>", "Provincia: ", "</b>", as.character(loretocondata$PROVINCIA.y), "<br>",
                "<b>", "Distrito: ", "</b>", as.character(loretocondata$DISTRITO.y), "<br>",
                "<b>", "Casos Positivos: ", "</b>", as.character(loretocondata$Casos_Positivos), "<br>")

palfac_loreto <- colorFactor(palette=c("#ffffcc","#a1dab4","#41b6c4","#225ea8","#636363"), domain = loretocondata$Categoria)

loreto_mapa <- leaflet(loretocondata) %>% addProviderTiles(providers$Esri.WorldGrayCanvas) %>% 
  addPolygons(color = "#444444", weight = 1, smoothFactor = 0.5, opacity = 1,
              fillOpacity = 0.5, fillColor = ~palfac_loreto(loretocondata$Categoria), group = "Belleza",
              highlightOptions = highlightOptions(color = "white", weight = 2, bringToFront = TRUE), 
              label = ~loretocondata$DISTRITO.y, labelOptions = labelOptions(direction = "auto"), popup = popup_loreto) %>%
  addLegend(position = "topright", pal = palfac_loreto, values = ~loretocondata$Categoria,
...
```{r La Libertad Data, include=FALSE}
distrito <- readOGR(dsn="/cloud/project/subnacional_files",layer="DISTRITOS")
datos_distrito=import("/cloud/project/subnacional_files/Seguimiento_Sub_nacional.xlsx", sheet="DatosDistrital_Mapa")
LL <- distrito[distrito$DEPARTAMEN == "LA LIBERTAD",]

LLcondata=merge(LL,datos_distrito,by.x="IDDIST",by.y="IDDIST")

LLcondata$fecha_actualizada<-as.Date(LLcondata$fecha_actualizada)

```


```{r La Libertad Codigo, include=FALSE}
popup_LL<- paste0("<b>", "Departamento: ", "</b>", as.character(LLcondata$DEPARTAMENTO), "<br>",
                "<b>", "Provincia: ", "</b>", as.character(LLcondata$PROVINCIA.y), "<br>",
                "<b>", "Distrito: ", "</b>", as.character(LLcondata$DISTRITO.y), "<br>",
                "<b>", "Casos Positivos: ", "</b>", as.character(LLcondata$Casos_Positivos), "<br>",
                "<b>", "Última actualización: ", "</b>", as.character(LLcondata$fecha_actualizada), "<br>")

palfac_LL <- colorFactor(palette=c("#ffffcc","#a1dab4","#41b6c4","#225ea8","#636363"), domain = LLcondata$Categoria)

LL_mapa <- leaflet(LLcondata) %>% addProviderTiles(providers$Esri.WorldGrayCanvas) %>% 
  addPolygons(color = "#444444", weight = 1, smoothFactor = 0.5, opacity = 1,
              fillOpacity = 0.5, fillColor = ~palfac_LL(LLcondata$Categoria), group = "Belleza",
              highlightOptions = highlightOptions(color = "white", weight = 2, bringToFront = TRUE), 
              label = ~LLcondata$DISTRITO.y, labelOptions = labelOptions(direction = "auto"), popup = popup_LL) %>%
  addLegend(position = "topright", pal = palfac_LL, values = ~LLcondata$Categoria,
            title = "% de casos positivos por Región")
```


```{r La Libertad Mapa, eval=TRUE, echo=FALSE, fig.width=3.5, fig.align='center', message=FALSE,warning=FALSE}
LL_mapa
```
